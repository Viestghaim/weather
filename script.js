		...
		var city = $("#city").val();
         ...
         if (city != '') {
              ...
              $.ajax({
                  ...
                  url: 'http://api.openweathermap.org/data/2.5/weather?q=' + city + '&units=metric' + '&appid=2f609d8da47eee89d9437370a427d47a',
                  type: "GET",
                  dataType: "jsonp",
         	...
         	$("#error").html("<div class='alert alert-danger id='errorCity'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Введи місто, Зеню! </div>");
         }
	});
});

function show (data) {
	return "<h3 style='font-size:40px; font-weight: bold;'>Поточна Погода для: " + data.name + "," + data.sys.country + "</h3>" +
